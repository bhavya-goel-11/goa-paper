...
// --- 3. CGOA2 Algorithm (Chaotic Initialization) ---
double run_cgoa2(int pop_size, int dim, int max_iter, double LB, double UB, int func_num) {
    double S = 88.0; 
    double PSRs = 0.34;
    ...
    // Chaotic Init Setup
    double C = rand_01();
    for(int k = 0; k < 30; k++) C = logistic_map(C); // Transient burn-in
    ...
    for (int i = 0; i < pop_size; i++) {
        ...
        for (int j = 0; j < dim; j++) {
            C = logistic_map(C); // Generate chaotic map position
            gazelle[i][j] = LB + C * (UB - LB);
        }
    ...
    // Optimization Loop (Standard GOA Math)
...
int main() {
    ...
    FILE *fp = fopen("../results/cgoa2_cec17_results.csv", "w");
    if (fp == NULL) {
    ...
    printf("Starting Mass Benchmark Test for CGOA2 (Chaotic Initialization)...\n\n");
    ...
    for (int func_num = 1; func_num <= 30; func_num++) {
        ...
        for (int run = 0; run < num_runs; run++) {
            double fit = run_cgoa2(pop_size, dim, max_iter, LB, UB, func_num);
            results[run] = fit;
            sum += fit;
    ...
    fclose(fp);
    printf("\nAll 30 runs completed successfully! Data saved to 'results/cgoa2_cec17_results.csv'.\n");
