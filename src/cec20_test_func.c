...
int ini_flag=0,n_flag,func_flag,*SS;
    ...
    //     printf("\n Input func_num %d Running fun num%d  \n", func_num0, func_num);
        ...
        if (ini_flag==0)
        {
            ...
            /* Load Matrix M*/
            sprintf(FileName, "../input_data/20/M_%d_D%d.txt", func_num,nx);
            ...
            /* Load shift_data */
            sprintf(FileName, "../input_data/20/shift_data_%d.txt", func_num);
            fpt = fopen(FileName,"r");
            if (fpt==NULL)
            {
            ...
            {
                sprintf(FileName, "../input_data/20/shuffle_data_%d_D%d.txt", func_num, nx);
                fpt = fopen(FileName,"r");
                if (fpt==NULL)
                {
...
void sum_diff_pow_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* sum of different power */
{
	...
	{
		double xi = z[i];
		//double newv = pow((abs(xi)),(i+1));   // Janez
