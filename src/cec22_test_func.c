...
void cec22_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "../input_data/22/M_%d_D%d.txt", func_num,nx);
		...
		if (func_num<9)
		{
			...
			for (i=0; i<nx*nx; i++)
			{
				int result; result=fscanf(fpt,"%lf",&M[i]); (void)result;
			}
		}
		else
		{
		...
		/* Load shift_data */
		sprintf(FileName, "../input_data/22/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
		...
		if (func_num<9)
		{
			...
			for(i=0;i<nx;i++)
			{
				int result; result=fscanf(fpt,"%lf",&OShift[i]); (void)result;
			}
		}
		else
		{
			...
			for(i=0;i<cf_num-1;i++)
			{
				for (j=0;j<nx;j++)
				{
					int result; result=fscanf(fpt,"%lf",&OShift[i*nx+j]); (void)result;

				}
				int result; result=fscanf(fpt,"%*[^\n]%*c"); (void)result;
			}
			for (j=0;j<nx;j++)
			{
 				int result; result=fscanf(fpt,"%lf",&OShift[nx*(cf_num-1)+j]); (void)result;
		...
		{
			sprintf(FileName, "../input_data/22/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
			...
			for(i=0;i<nx;i++)
			{
				int result; result=fscanf(fpt,"%d",&SS[i]); (void)result;
			}
			fclose(fpt);
		}
